...
export class CashSystemBasket {
  ...
  public updatePrice() {
    ...
    for (let i = 0; i < this.items.length; i++) {
      ...
      this.priceHandler.priceTotal +=
        item.active && item.finalized == 0 ? item.price : 0;
    ...
    for (let i = 0; i < this.payments.length; i++) {
      ...
      this.priceHandler.priceGiven +=
        payment.finalized == 0 ? payment.price : 0;
      /*
    ...
    this.priceHandler.priceOpen =
      this.priceHandler.priceTotal - this.priceHandler.pricePayed;
    ...
    if (this.itemIsUnique(item)) {
      if (item.lineId == 1 || !item.lineId)
        item.lineId = this.items[0]
          ? 1 + this.items[this.items.length - 1].lineId
          : item.lineId;
  ...
  private itemIsUnique(item: ItemTransaction) {
    ...
    const eq = this.items.find(
      (bI) =>
        ...
        bI.price ===
          item.price /*&& bI.amount === 1 ||  bI.amount > 1 && bI.priceSingle === item.priceSingle  */ &&
        bI.title === item.title &&
