    ...
    if (payload.userId == request.user.userId) {
      const query = await this.connection.query(
        `
         ...
         left join com_user_roles cur on com_company.companyId = cur.companyId
         where cur.userId = ? and com_company.companyId = ? limit 1
     `,
        [payload.userId, payload.companyId]
      );
      if (query && query[0]) {
        request._company = query[0]; // assign business/company obj to req
        return payload.companyId == request._company.companyId;
      }
    }
